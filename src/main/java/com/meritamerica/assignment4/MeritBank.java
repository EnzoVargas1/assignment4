	...
	public static boolean readFromFile(String fileName)  {
		try {
			...
			for (int i = 0; i < accHolderLength; i++) {
				...
				for (int j = 0; j < unprocessedTransactions.size(); j++) {
					Transaction transact = Transaction.readFromString(unprocessedTransactions.get(j));
			...
			int fraudQueuePendingNum = Integer.parseInt(line);
			
			for(int i=0;i<fraudQueuePendingNum;i++) {
				line=rd.readLine();
				StringTokenizer token = new StringTokenizer(line, ",");
				long sourceAccNum =Integer.parseInt(token.nextToken());
				long targetAccNum =Integer.parseInt(token.nextToken());
				double amount = Double.parseDouble(token.nextToken());
				Date date = new Date(token.nextToken());
				if(sourceAccNum>0) {
					TransferTransaction transferTransact = new TransferTransaction(getBankAccount(sourceAccNum),getBankAccount(targetAccNum),amount);
        			getFraudQueue().addTransaction(transferTransact);
                }
				if(sourceAccNum==-1) {
					if(amount>0){
						DepositTransaction depositTransact = new DepositTransaction(getBankAccount(targetAccNum),amount);
						getFraudQueue().addTransaction(depositTransact);
					}
					if(amount<0) {
						WithdrawTransaction withdrawTransact = new WithdrawTransaction(getBankAccount(targetAccNum),amount);
						getFraudQueue().addTransaction(withdrawTransact);
					}
				}
			}
			rd.close();
			...
			for(int i=0;i<accHolderList.size();i++) {
				...
				if(numOfCheckings!=0) {
					...
					for(int j=0;j<numOfCheckings;j++) {
						...
						int numOfTransactions = checking[j].getTransactions().size();
						
						wr.println(numOfTransactions);
						List<Transaction> transact = checking[j].getTransactions();
						for(int k=0;k<numOfTransactions;k++) {
							wr.println(transact.get(k).writeToString());
						}
					}
				}
